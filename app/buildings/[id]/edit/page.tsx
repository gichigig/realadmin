...
export default function EditBuildingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAuthenticated || !user) {
      alert("Please login to edit a building");
      router.push("/login");
      return;
    }
    ...
    try {
      ...
      await buildingsApi.update(buildingId, updatedBuilding);
      router.push("/buildings");
    } catch (error) {
      console.error("Failed to update building:", error);
      alert(error instanceof Error ? error.message : "Failed to update building. Make sure the backend is running.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-gray-500 text-sm animate-pulse">Loading building profile...</p>
      </div>
    );
  }
      ...
      {/* Navigation & Header */}
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 text-sm mt-1">
            Update the complex name, address, ward, and coordinates. Associated units will inherit the new location data.
          </p>
        </div>
