...
export default function HelperDashboard() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!isAuthenticated) {
        router.push("/login");
      } else if (!isSuperAdmin && user?.primaryRole && user.primaryRole !== "helper") {
        router.push(user.primaryRole === "services" ? "/services" : "/");
      }
    }
  ...
  const fetchDashboard = async () => {
    try {
      const res = await helperApi.getDashboard();
      setData(res);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isNaN(parsed) || parsed < 0) throw new Error("Invalid price");
      await helperApi.updateProfile({ 
        price: parsed, 
        county: countyInput,
        coverageLevel,
        constituencies,
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await helperApi.withdraw(amount);
  ...
  if (!isSuperAdmin && user?.primaryRole !== "helper" && userRole !== "ADMIN" && userRole !== "SUPER_ADMIN") {
    return (
      <div className="p-8 text-center text-red-600 flex flex-col items-center">
        <p className="text-lg font-bold mb-2">You do not have access to the Helper Dashboard.</p>
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Manage your earnings, set your rates, and withdraw to M-Pesa.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-blue-100 rounded-lg"><CurrencyDollarIcon className="w-6 h-6 text-blue-600" /></div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-purple-100 rounded-lg"><DocumentTextIcon className="w-6 h-6 text-purple-600" /></div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
                  ...
                  {['COUNTY', 'CONSTITUENCY', 'WARD'].map(level => (
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-md transition-colors ${
                        coverageLevel === level ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"
                      }`}
                    >
                      {level}
                    </button>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setConstituencies([]); // reset children
                    setWards([]);
                  }}
              ...
              {coverageLevel === 'CONSTITUENCY' && countyInput && (
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-white min-h-[100px]"
                    required
                  >
              ...
              {coverageLevel === 'WARD' && countyInput && (
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-white min-h-[150px]"
                    required
                  >
                    {locationTree?.constituenciesByCounty[countyInput]?.map((c: string) => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Your Fee Rate (KES)</label>
                <input
                  type="number"
                  value={priceInput}
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
            ...
            <form onSubmit={handleWithdraw} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-4 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 disabled:opacity-50 transition-colors"
              >
                    ...
                    <li key={job.id} className="p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex justify-between items-start mb-1">
                        <span className="font-medium text-gray-900">
                          {job.description || "Helper Service"}
                        </span>
